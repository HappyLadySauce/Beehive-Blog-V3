...
const fileServicePrefix = "/file.FileService/"
...
func (i *InternalAuthInterceptor) Unary() grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		if info == nil || !strings.HasPrefix(info.FullMethod, fileServicePrefix) {
			return handler(ctx, req)
		}
		caller, authErr := i.authenticate(ctx)
		if authErr != nil {
			logs.Ctx(ctx).Warn(
				"file_internal_auth_failed",
				logs.String("caller", caller),
				logs.RequestID(ctxmeta.GetRequestIDFromIncomingContext(ctx)),
				logs.Err(authErr),
			)
			return nil, errgrpcx.ToStatusWithFallback(authErr, errs.CodeFileInternal, "internal caller authentication failed")
		}
...
func (i *InternalAuthInterceptor) authenticate(ctx context.Context) (string, error) {
	...
	if !ok {
		return "", errs.New(errs.CodeFileInternalCallerUnauthorized, "internal caller authentication failed")
	}
	...
	if token == "" || caller == "" {
		return caller, errs.New(errs.CodeFileInternalCallerUnauthorized, "internal caller authentication failed")
	}
