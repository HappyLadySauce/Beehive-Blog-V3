...
func (m *Manager) ListAssets(ctx context.Context, in ListAssetsInput) (*AssetListResult, error) {
	...
	scope, err := normalizeOptionalScope(in.Scope)
	if err != nil {
		return nil, err
	}
	...
	assets, total, err := m.store.Assets.List(ctx, repoAssetFilter(ownerUserID, scope, status, visibility, in.Keyword, in.Page, in.PageSize))
	if err != nil {
...
func repoAssetFilter(ownerUserID int64, scope string, status string, visibility string, keyword string, page int, pageSize int) repo.AssetListFilter {
